...
export class V0_1_0_OwnedCards implements IBaseMigration {
  ...
  up(db: Kysely<any>): Promise<void> {
    return createV0_1_0_CardCondition(db)
      .then(() => populateV0_0_1_CardCondition(db))
      .then(() => createV0_1_0_OwnedCard(db))
      .then(() => createV0_1_0_OwnedCardCollectionMap(db));
  }

  down(db: Kysely<any>): Promise<void> {
    return db.schema.dropTable("owned_card_collection_map").execute()
      .then(async () => await db.schema.dropTable("card_condition").execute())
      .then(async () => await db.schema.dropTable("owned_card").execute());
  }
}

function createV0_1_0_CardCondition(db: Kysely<any>): Promise<void> {
 ...
 */
function populateV0_0_1_CardCondition(db: Kysely<any>) {
  const values = new Array<Insertable<CardConditionTable>>();
  const now = sqliteUTCTimeStamp();

  values.push({
    created_at: now, modified_at: now, id: "MT", color_code: "", sequence: 0, expression: "Mint", condition: "Perfect", us_expression: "Mint", description:
      "<p>A mint card is in perfect condition; no excuses. This means, that the front is in perfect condition, there are no scratches on the surface, and the surface is perfectly clean. For the back it means, that the card is indistinguishable from cards of a newly openend booster. If a card has a signature or a Grand Prix stamp it can never be graded Mint, even if the card stock is otherwise in Mint condition.</p>" +
  ...
  values.push({
    created_at: now, modified_at: now, id: "NM", color_code: "", sequence: 1, expression: "Near Mint", condition: "Pack-Fresh", us_expression: "Near Mnt", description:
      "<p>A Near Mint card looks like it has never been played without sleeves. Small allowances can be made, but the card generally shows no wear.</p>" +
  ...
  values.push({
    created_at: now, modified_at: now, id: "EX", color_code: "", sequence: 2, expression: "Excellent", condition: "Minor Wear", us_expression: "Slightly Played | Excellent", description:
      "<p>An Excellent card looks like it was used for a few games without sleeves. For Excellent cards it is almost always clearly visible upon first inspection that the card is not in perfect condition. However, although the damage is clearly visible it is only of minor severity.</p>" +
  ...
  values.push({
    created_at: now, modified_at: now, id: "GD", color_code: "", sequence: 3, expression: "Good", condition: "Visible Wear", us_expression: "Moderately Played | Very Good", description:
      "<p>A Good card looks like it might have been used for a long tournament without sleeves.</p>" +
  ...
  values.push({
    created_at: now, modified_at: now, id: "LP", color_code: "", sequence: 4, expression: "Light Played", condition: "Severe Wear", us_expression: "Played | Good", description:
      "<p>A Light Played card looks as if it has been used without sleeves for an extended period of time.</p>" +
  ...
  values.push({
    created_at: now, modified_at: now, id: "PL", color_code: "", sequence: 5, expression: "Played", condition: "Damaged", us_expression: "Heavily Played", description:
      "<p>A Played card looks as bad as you can get a card through regular use without sleeves.</p>" +
  ...
  values.push({
    created_at: now, modified_at: now, id: "PO", color_code: "", sequence: 6, expression: "Poor", condition: "Destroyed", us_expression: "Poort", description:
      "<p>A Poor card has damage that cannot normally have stemmed from regular use of the card.</p>" +
