...
async function populateV1_0_0_Color(db: Kysely<any>): Promise<Array<InsertResult>> {
  ...
  values.push({ created_at: now, id: "U", sequence: 1, display_text: "Blue", land_type: "Island", mana_symbol: "{U}" });
  values.push({ created_at: now, id: "B", sequence: 2, display_text: "Black", land_type: "Swamp", mana_symbol: "{B}" });
  values.push({ created_at: now, id: "R", sequence: 3, display_text: "Red", land_type: "Mountain", mana_symbol: "{R}" });
  values.push({ created_at: now, id: "G", sequence: 4, display_text: "Green", land_type: "Forest", mana_symbol: "{G}" });
  values.push({ created_at: now, id: "C", sequence: 5, display_text: "Colorless", mana_symbol: "{C}" });
  return db
    .insertInto("color")
    .values(values)
    .execute();
